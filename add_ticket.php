...
if (isset($_GET['nagiosTicket']) && $_GET['nagiosTicket'] == 881234123 || $_GET['newTicketType'] == "new") {
	if (isset($_GET['nagiosTicket']) && $_GET['nagiosTicket'] == 881234123) {// This is a ticket generated by nagios
		$_GET["newTicketUser_id"] = 128;
		$_GET["newTicketType"] = "new";
	...
	}// end of nagios ticket
	
	if ($_GET["newTicketUser_id"] == 0 || $_GET["newTicketUser_id"] == "") { $_GET["newTicketUser_id"] = $usr -> User_id;} // there was some kind of bug finding the user info for who created the ticket
	// lets see if we can fix this issue
	if ($_GET["newTicketBugTrouble"] == 1 && $ok === true) {// we are going to insert a new ticket
		...
		$newTicketId = $db -> Query('INSERT INTO tickets(created_by_id,assigned_by_id,assigned_id,category_id,subject,description,created_on,open,priority,due_on,location,tickettype_id) 
		VALUES(
	...
	$db -> Query("UPDATE tickets SET status='" . $res1['status'] . "' WHERE id=" . $_GET['newTicketTicket_id'] . " LIMIT 1;");
	//put in the new status
	//end status area
}
