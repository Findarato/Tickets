...
if (isset($_GET["area"])) {
  switch($_GET["area"]) {
    ...
    case "sOpen" :
      ...
      break;
    case "sClosed" :
      // closed tickets that I am invloved with {Closed Tickets}
      $sql = "SELECT 
      t.id
      FROM tickets AS t 
    ...
    case "closedDepartment" :
      // closed tickets that my Department is involved in
      ...
      break;
    case "sOdepartment" :
      // Assigned to people in my department {To My Department}
      ...
      break;
    case "sAdepartment" :
      // Assigned to people in my department {By My Department}
      $depIds = getDepartmentMembers_by_userid($usr -> User_id);
      $sql = "SELECT 
      t.id
      FROM tickets AS t 
      ...
      break;
    case "sAssigned" :
      // created by me, or assigned by me {By Me}
      $sql = "SELECT 
      t.id
      FROM tickets AS t 
      WHERE (t.created_by_id=" . $usr -> User_id . "
    ...
    case "new" :
      ...
      if (!is_array($replyTicketids)) {$replyTicketids = array(0 => $replyTicketids);
      } else {$replyTicketids = array_implode($replyTicketids);
      }
      ...
      } else {$ticketIds = array_implode($ticketIds);
      }

      if (!is_array($responses)) {
        if ($response == "") {$responses = 0;
        }$responses = array(0 => $responses);
      } else {$responses = array_implode($responses);
      }
  ...
  if (isset($_GET['direction']) && ($_GET['direction'] == "asc" || $_GET['direction'] == "desc")) {$dir = $_GET['direction'];
  } else {$dir = 'DESC';
  }
  switch($sort) {
    case 'title' :
